...
void __main(void) noexcept
{
	...
	asm volatile(
		...
		"str %[cm7_init_vtor], %[vtor];" // Accepts the same value as GPR16, but ignores the lower 7 bits, so it's better here.
		...
		: [flexram_bank_cfg_sel] "ir"(0x00000007), [cm7_init_vtor_bic] "ir"(0x7F), [cm7_init_vtor] "r"(&__interrupt_vector_table)
		: "r0", "memory");

	// This line breaks the compiled binary, so we skip it. Teensyduino does this safely somehow?
